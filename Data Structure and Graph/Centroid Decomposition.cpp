...
void get_cnts(int u, int p, bool f, int depth = 1) {
  ...
  if (f) cnt[depth]++; else ans += cnt[k - depth];
  for (auto v : adj[u]) {
    if (v == p || is_removed[v]) continue;
    get_cnts(v, u, f, depth + 1);
  }
}
void get_cnts2(int sum, int u, p, bool f, int depth = 1) {
  if (sum > k) return; if (f) cnt[sum] = min(cnt[sum], depth);
  else ans = min(ans, depth + cnt[k - sum]);
  for (auto [v, w] : adj[u]) {
    if (v == p || is_removed[v]) continue;
    get_cnts2(sum + w, v, u, f, depth + 1);
  }
}
void del(int sum, int u, int p) {
  if (sum > k) return;
  cnt[sum] = 2e7; for (auto [v, w] : adj[u]) {
    if (v == p || is_removed[v]) continue;
    del(sum + w, v, u);
  }
}
void build_cd(int u = 1) {
  ...
  for (auto [v, w] : adj[c]) {
    if (is_removed[v]) continue;
    get_cnts2(w, v, c, 0); get_cnts2(w, v, c, 1);
  }
  for (auto [v, w] : adj[c]) {
    if (is_removed[v]) continue; del(w, v, c);
  }
  //end-cal
  is_removed[c] = 1;
  for (auto v : adj[c]) {
